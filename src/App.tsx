...
export default function App() {
  ...
  const handleApplySingleMaterial = (role: SupportedMaterialRole, material: EngineeringMaterial) => {
    ...
    setActivityLogs(prev => [
      {
        id: Math.random().toString(36).substring(2, 9),
        timestamp: new Date(),
        descriptionAr: `✓ قبول توصية مادة: تم تطبيق (${material.name}) على خلطة المشروع بنجاح`,
        descriptionFr: `✓ Matériau appliqué : (${material.name}) ajouté au mélange`,
        descriptionEn: `✓ Material applied: (${material.name}) added to mix`,
        type: "success"
      },
      ...prev
    ].slice(0, 30));
    ...
    setNotifications(prev => [
      {
        id: String(Date.now()),
        textAr: `تم قبول وتطبيق المادة (${material.name}) بنجاح!`,
        textFr: `Matériau (${material.name}) appliqué avec succès !`,
                            ...
                            {isCementAllowed && (
                              <div className="p-3 bg-slate-50 dark:bg-slate-900/40 rounded-xl border border-slate-200/40 dark:border-slate-800 space-y-2.5">
                                ...
                                <div>
                                  ...
                                  >
                                    <option value="">{language === "ar" ? "اختر مادة من المستودع (المعتمدة فقط)" : language === "fr" ? "Choisir un matériau approuvé" : "Select approved material"}</option>
                                    {cementList.map(m => (
                                ...
                                {cementList.length === 0 && (
                                  <div className="mt-2 p-2 bg-amber-500/10 border border-amber-500/20 rounded-lg text-[10px] text-amber-700 dark:text-amber-300 font-bold">
                                    ⚠️ {language === "ar" ? "لا توجد مواد إسمنت مسجلة في المستودع." : "No cement materials in repository."}
                                  </div>
                                )}

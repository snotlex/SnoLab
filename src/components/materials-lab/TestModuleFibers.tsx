...
import { calculateFiberProperties } from "../../utils/materialTestingCalculators";

interface TestModuleFibersProps {
...
export const TestModuleFibers: React.FC<TestModuleFibersProps> = ({
  material,
  operator,
  ...
  const [sampleId, setSampleId] = useState(`SMP-FIB-${Date.now().toString().slice(-4)}`);
  const [testDate, setTestDate] = useState(new Date().toISOString().split("T")[0]);
  const [notes, setNotes] = useState("");

  const [fiberType, setFiberType] = useState<"steel" | "polypropylene" | "glass" | "synthetic">("steel");
  const [lengthMm, setLengthMm] = useState<number>(50); // mm
  const [diameterMm, setDiameterMm] = useState<number>(0.9); // mm
  const [tensileStrength, setTensileStrength] = useState<number>(1150); // MPa
  const [youngModulus, setYoungModulus] = useState<number>(210); // GPa
  const [density, setDensity] = useState<number>(7.85); // g/cm³
  const [recommendedDosage, setRecommendedDosage] = useState<number>(30); // kg/m³

  const computed = useMemo(() => {
    const res = calculateFiberProperties(fiberType, lengthMm, diameterMm, tensileStrength, youngModulus, density, recommendedDosage);
    return {
      titleAr: "توصيف الألياف والخواص الميكانيكية (Fibers Characterization)",
      titleFr: "Caractérisation mécanique des fibres selon NF EN 14889",
      titleEn: "Concrete Reinforcement Fibers Characterization",
      standard: "NF EN 14889-1 / ASTM A820",
      results: res,
      status: res.status,
      score: res.status === "PASS" ? 97 : 75,
  ...
  }, [fiberType, lengthMm, diameterMm, tensileStrength, youngModulus, density, recommendedDosage]);

  const handleSave = () => {
    const record: MaterialTestRecord = {
      id: `TEST-FIB-${Date.now().toString().slice(-6)}`,
      testType: "FIBER_CHARACTERIZATION",
      ...
      inputs: { fiberType, lengthMm, diameterMm, tensileStrength, youngModulus, density, recommendedDosage },
      ...
      <div className="p-4 rounded-2xl bg-slate-900 text-white border border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            ألياف التسليح: <span className="text-white font-bold">{material.name}</span>
          </p>
        </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
            <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
              ...
              <span>القياسات الهندسية للألياف (EN 14889)</span>
            </h3>

            <div className="space-y-3 text-xs">
              <div>
                <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">نوع الألياف</label>
                <select
                  value={fiberType}
                ...
                >
                  ...
                  <option value="polypropylene">ألياف بولي بروبيلين (Polypropylene Fibers)</option>
                  <option value="glass">ألياف زجاجية (Glass Fibers)</option>
                  <option value="synthetic">ألياف بوليميرية مصنعة (Macro Synthetic)</option>
                </select>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">الطول L (mm)</label>
                  <input
                    type="number"
                    value={lengthMm}
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">القطر d (mm)</label>
                  <input
                    type="number"
                    step="0.05"
                    value={diameterMm}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">مقاومة الشد (MPa)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">معامل المرونة (GPa)</label>
                  <input
                    type="number"
                    value={youngModulus}
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex gap-2">
              ...
              >
                <Save size={15} />
                <span>حفظ نتائج فحص الألياف</span>
              </button>
              <button
                onClick={onCancel}
          ...
          <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>جدول مطابقة ألياف التسليح (NF EN 14889)</span>
              </span>
              <span className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-0.5 rounded-full">
                L/d = {computed.results.aspectRatio}
              </span>
            </div>
              ...
              <table className="w-full text-right">
                <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-500 font-bold">
                  <tr>
                    ...
                    <th className="py-2 px-3">المعيار</th>

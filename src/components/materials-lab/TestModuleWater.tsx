...
import { calculateWaterQuality } from "../../utils/materialTestingCalculators";

interface TestModuleWaterProps {
...
export const TestModuleWater: React.FC<TestModuleWaterProps> = ({
  material,
  operator,
  ...
  const [sampleId, setSampleId] = useState(`SMP-WAT-${Date.now().toString().slice(-4)}`);
  const [testDate, setTestDate] = useState(new Date().toISOString().split("T")[0]);
  const [notes, setNotes] = useState("");

  const [pH, setPH] = useState<number>(7.4);
  const [chlorides, setChlorides] = useState<number>(140); // mg/L
  const [sulfates, setSulfates] = useState<number>(210); // mg/L
  const [tds, setTds] = useState<number>(480); // mg/L
  const [organics, setOrganics] = useState<number>(15); // mg/L

  const computed = useMemo(() => {
    const res = calculateWaterQuality(pH, chlorides, sulfates, tds, organics);
    return {
      titleAr: "فحص جودة ومطابقة مياه الخلط (Mixing Water Quality)",
      titleFr: "Conformité de l'eau de gâchage EN 1008",
      titleEn: "Mixing Water Quality & Chemical Analysis",
      standard: "NF EN 1008 / ISO 10523",
      results: res,
      status: res.status,
      score: res.status === "PASS" ? 100 : res.status === "WARNING" ? 80 : 40,
  ...
  }, [pH, chlorides, sulfates, tds, organics]);

  const handleSave = () => {
    const record: MaterialTestRecord = {
      id: `TEST-WAT-${Date.now().toString().slice(-6)}`,
      ...
      inputs: { pH, chlorides, sulfates, tds, organics },
      ...
      <div className="p-4 rounded-2xl bg-slate-900 text-white border border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            مصدر المياه: <span className="text-white font-bold">{material.name}</span>
          </p>
        </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
            <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
              ...
              <span>التحاليل الكيميائية لماء الخلط (EN 1008)</span>
            </h3>
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">
                  محتوى الكلوريدات Cl⁻ (mg/L):
                </label>
                <input
                  type="number"
                  value={chlorides}
                  onChange={(e) => setChlorides(Number(e.target.value))}
                  className="w-full px-3 py-1.5 rounded-lg border border-slate-300 dark:border-slate-700 font-mono font-bold"
                />
              </div>
              ...
              <div>
                <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">
                  محتوى الكبريتات SO₄²⁻ (mg/L):
                </label>
                <input
                  type="number"
                  value={sulfates}
                  onChange={(e) => setSulfates(Number(e.target.value))}
                  className="w-full px-3 py-1.5 rounded-lg border border-slate-300 dark:border-slate-700 font-mono font-bold"
                />
              </div>
              ...
              <div>
                <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">
                  المواد الصلبة الذائبة الكلية TDS (mg/L):
                </label>
                <input
                  type="number"
                  value={tds}
          ...
          <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>جدول مطابقة مياه الخرسانة (NF EN 1008)</span>
              </span>
              <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-black ${
                ...
                {computed.status === "PASS" ? <CheckCircle size={12} /> : <XCircle size={12} />}
                <span>{computed.status === "PASS" ? "مياه صالحة ومعتمدة" : "مياه غير مطابقة"}</span>
              </span>
            </div>
              ...
              <table className="w-full text-right">
                <thead className="bg-slate-50 dark:bg-slate-800/80 text-slate-500 font-bold">
                  <tr>
                    ...
                    <th className="py-2 px-3">التركيز المقاس</th>
                    <th className="py-2 px-3">الحد الأقصى (EN 1008)</th>

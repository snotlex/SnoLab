...
import { calculateAdmixtureProperties } from "../../utils/materialTestingCalculators";

interface TestModuleAdmixturesProps {
...
export const TestModuleAdmixtures: React.FC<TestModuleAdmixturesProps> = ({
  material,
  operator,
  ...
  const [sampleId, setSampleId] = useState(`SMP-ADM-${Date.now().toString().slice(-4)}`);
  const [testDate, setTestDate] = useState(new Date().toISOString().split("T")[0]);
  const [notes, setNotes] = useState("");

  const [density, setDensity] = useState<number>(1.08); // g/cm³
  const [pH, setPH] = useState<number>(5.8);
  const [solidContent, setSolidContent] = useState<number>(32.0); // %
  const [waterReduction, setWaterReduction] = useState<number>(24.5); // %
  const [settingTimeDiff, setSettingTimeDiff] = useState<number>(15); // min

  const computed = useMemo(() => {
    const res = calculateAdmixtureProperties(density, pH, solidContent, waterReduction, settingTimeDiff);
    return {
      titleAr: "توصيف المضاف الكيميائي ومعدل تخفيض الماء (Chemical Admixture Test)",
      titleFr: "Caractérisation de l'adjuvant selon NF EN 934-2",
      titleEn: "Chemical Admixture Performance & Characterization",
      standard: "NF EN 934-2 / NF EN 480",
      results: res,
      status: res.status,
      score: res.status === "PASS" ? 97 : 80,
      interpretation: res.interpretation,
      compliance: res.compliance,
      syncedProps: {
        density: density * 1000,
        waterReduction,
        settingTimeImpact: settingTimeDiff
      }
    };
  }, [density, pH, solidContent, waterReduction, settingTimeDiff]);

  const handleSave = () => {
    const record: MaterialTestRecord = {
      id: `TEST-ADM-${Date.now().toString().slice(-6)}`,
      ...
      inputs: { density, pH, solidContent, waterReduction, settingTimeDiff },
      ...
      <div className="p-4 rounded-2xl bg-slate-900 text-white border border-slate-800 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            المضاف الكيميائي: <span className="text-white font-bold">{material.name}</span>
          </p>
        </div>
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
            <h3 className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-2 border-b border-slate-100 dark:border-slate-800 pb-2">
              ...
              <span>نتائج الفحص المخبري للمضاف الكيميائي</span>
            </h3>

            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">الكثافة النسبية (g/cm³)</label>
                <input
                  type="number"
                  step="0.01"
                  value={density}
              ...
              <div>
                <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">درجة الحموضة (pH)</label>
                <input
                  type="number"
                  step="0.1"
                  value={pH}
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div>
                <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">المحتوى الصلب (%)</label>
                <input
                  type="number"
                  step="0.1"
                  value={solidContent}
                  onChange={(e) => setSolidContent(Number(e.target.value))}
                  className="w-full px-3 py-1.5 rounded-lg border border-slate-300 dark:border-slate-700 font-mono font-bold"
                />
              </div>
              <div>
                <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">تخفيض ماء الخلط (%)</label>
                <input
                  type="number"
                  step="0.1"
            ...
            <div className="text-xs">
              <label className="text-slate-600 dark:text-slate-400 font-semibold mb-1 block">
                فارق زمن الشك مقارنة بالخرسانة المرجعية (دقيقة):
              </label>
              <input
                type="number"
                value={settingTimeDiff}
                onChange={(e) => setSettingTimeDiff(Number(e.target.value))}
                className="w-full px-3 py-1.5 rounded-lg border border-slate-300 dark:border-slate-700 font-mono font-bold"
              />
            </div>
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800 flex gap-2">
              ...
              >
                <Save size={15} />
                <span>حفظ نتائج فحص المضاف</span>
              </button>
              <button
                onClick={onCancel}
          ...
          <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black text-slate-900 dark:text-white uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span>جدول مطابقة المضاف الكيميائي (NF EN 934-2)</span>
              </span>
              <span className="text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2.5 py-0.5 rounded-full">
                {computed.results.waterReductionPercent}% Water Reduction
              </span>
            </div>
